...
public class cashierLogs implements Initializable {
    ...
    @FXML
    private TableColumn<LoginLogRow, String> customerCol;
    @FXML
    ...
    private void setupTable() {
        customerCol.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getName()));
        ...
        logsTable.setPlaceholder(new Label("No customer login records yet."));
    }
    ...
    private void loadLogs() {
        ...
        String sql = "SELECT log_id, COALESCE(u_name, 'Unknown') AS u_name, "
                + "COALESCE(u_email, '-') AS u_email, login_at, COALESCE(logout_at, '') AS logout_at "
                + "FROM tbl_login_logs "
                + "WHERE LOWER(COALESCE(u_role, '')) = 'user' "
        ...
        totalLogsMetaLabel.setText(
                "Customer Sessions: " + String.format("%,d", rows.size()) + " | Active: " + String.format("%,d", activeCount));
    }
